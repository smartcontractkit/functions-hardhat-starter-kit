...
const verifyOffchainSecrets = async (secretsURLs, nodeAddresses) => {
  ...
  for (const { secrets, url } of offchainSecretsResponses) {
    ...
    for (const nodeAddress of nodeAddresses) {
      if (!secrets[nodeAddress]) {
        if (!secrets["0x0"]) {
          throw Error(`No secrets specified for node ${nodeAddress} and no default secrets found.`)
        }
        console.log(`WARNING: No secrets found for node ${nodeAddress}.  That node will use default secrets specified by the "0x0" entry.`)
      }
    }
  }
}