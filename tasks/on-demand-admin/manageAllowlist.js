...
async function addOrRemove(action, taskArgs) {
  ...
  if (action == Action.Add) {
    console.log(`Adding addresses ${addresses} to oracle ${networkConfig[network.name]['ocr2drOracle']}`)
    tx = await oracle.addAuthorizedSenders(addresses)
  } else {
    console.log(`Removing addresses ${addresses} from oracle ${networkConfig[network.name]['ocr2drOracle']}`)
