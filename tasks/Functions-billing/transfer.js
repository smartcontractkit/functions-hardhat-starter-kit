...
Any previous encrypted secrets will no longer work with subscription ID ${subId} and must be regenerated by the new owner.`
    )

    const sm = new SubscriptionManager({ signer, linkTokenAddress, functionsRouterAddress })
    await sm.initialize()

    console.log(`\nRequesting transfer of subscription ${subId} to new owner ${newOwner}`)
    const requestTransferTx = await sm.requestSubscriptionTransfer({ subId, newOwner, txOptions })
    console.log(
