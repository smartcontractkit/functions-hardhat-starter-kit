...
import "./FunctionsRegistryInterface.sol";

/**
 * @title Functions oracle interface.
 */
interface FunctionsOracleInterface {
  /**
   * @notice Gets the stored billing registry address
   * @return registryAddress The address of Functions billing registry contract
   */
  function getRegistry() external view returns (address);

  /**
   * @notice Sets the stored billing registry address
   * @param registryAddress The address of Functions billing registry contract
   */
   ...
   */
  function getRequiredFee(bytes calldata data, FunctionsRegistryInterface.RequestBilling calldata billing)
    external
    view
    returns (uint96);
